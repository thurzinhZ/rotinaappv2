...
export default function App() {
  ...
  const [supaUrl, setSupaUrl] = useState('https://jrspadoowijuqfzvhngd.supabase.co');
  const [supaKey, setSupaKey] = useState('eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Impyc3BhZG9vd2lqdXFmenZobmdkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI3NDE2MzksImV4cCI6MjA5ODMxNzYzOX0.0H8eW-w6doWmgBb3MGQO-5YGbsD6_A3fohMLxzmaXTk');
  const [useSupa, setUseSupa] = useState(true);
  ...
  useEffect(() => {
    async function loadAppSession() {
      try {
        ...
        // Forçar uso do Supabase real com as credenciais padrão do usuário
        setSupaUrl('https://jrspadoowijuqfzvhngd.supabase.co');
        setSupaKey('eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Impyc3BhZG9vd2lqdXFmenZobmdkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI3NDE2MzksImV4cCI6MjA5ODMxNzYzOX0.0H8eW-w6doWmgBb3MGQO-5YGbsD6_A3fohMLxzmaXTk');
        setUseSupa(true);
        await initializeCustomSupabase(
          'https://jrspadoowijuqfzvhngd.supabase.co',
          'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Impyc3BhZG9vd2lqdXFmenZobmdkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI3NDE2MzksImV4cCI6MjA5ODMxNzYzOX0.0H8eW-w6doWmgBb3MGQO-5YGbsD6_A3fohMLxzmaXTk'
        );
  ...
  const loadTasks = async (userId) => {
    try {
      if (useSupa && supabase) {
        ...
        // Mapeia de latitude/longitude para lat/lng usado no aplicativo
        const mappedTasks = (data || []).map(t => ({
          ...t,
          lat: t.latitude !== undefined ? Number(t.latitude) : t.lat,
          lng: t.longitude !== undefined ? Number(t.longitude) : t.lng,
        }));
  ...
  const saveTask = async () => {
    ...
    try {
      ...
      const taskData = {
        id: isEditing ? editingTask.id : generateUUID(), // <-- AGORA USA UUID REAL
      ...
      if (useSupa && supabase) {
        ...
        if (isEditing) {
          const { error } = await supabase
            ...
            .update({
              ...
              latitude: taskData.lat,     // <-- MAPEADO PARA LATITUDE
              longitude: taskData.lng,    // <-- MAPEADO PARA LONGITUDE
          ...
          // Criar cópia mapeada para as colunas reais da tabela (latitude/longitude)
          const dbTaskData = {
            id: taskData.id,
            title: taskData.title,
            description: taskData.description,
        ...
        // Obter as tarefas locais atuais
        const localData = await AsyncStorage.getItem(LOCAL_TASKS_KEY);
      ...
      Alert.alert('Sucesso', isEditing ? 'Tarefa atualizada com sucesso!' : 'Tarefa criada com sucesso!');
      ...
      Alert.alert('Erro', 'Não foi possível salvar a tarefa: ' + (err.message || err));
    }
  };
        ...
        {activeTab === 'config' && (
          <ScrollView contentContainerStyle={styles.configContainer}>
            ...
            <Text style={styles.configSubtitle}>
              O RotinaApp está configurado para salvar e sincronizar todos os dados de forma persistente diretamente no banco de dados do Supabase.
            </Text>
            ...
            <View style={styles.configCard}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ fontWeight: 'bold', fontSize: 14, color: '#1e293b' }}>Supabase Cloud Oficial</Text>
                </View>
              </View>
              <Text style={{ fontSize: 12, color: '#64748b', lineHeight: 16, marginBottom: 12 }}>
                O aplicativo está sincronizando diretamente com seu banco de dados na nuvem Supabase. O simulador offline foi removido.
              </Text>
              <View style={{ backgroundColor: '#f1f5f9', padding: 10, borderRadius: 12, gap: 4 }}>
                <Text style={{ fontSize: 9, fontWeight: 'bold', color: '#64748b' }}>URL DO BANCO:</Text>
                <Text style={{ fontSize: 10, color: '#334155' }}>https://jrspadoowijuqfzvhngd.supabase.co</Text>
              </View>
            </View>
